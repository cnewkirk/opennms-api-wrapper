...
- Corrected mocked response fixtures that disagreed with live servers
  (verified against Horizon 33.0.0, 36.0.3, and the Meridian 2025 and
  2026 foundations during the go-opennms port's validation sweep).
  The wrapper itself already returned the live shapes — only the test
  fixtures were wrong:
  - `GET /rest/stats/alarms/by-severity`, `GET /rest/graphs`, BSM
    map/reduce function lists, and v2 node metadata return wrapped
    objects, not bare arrays.
  - `GET /rest/flows/exporters` returns a bare array; `GET /rest/scv`
    returns a bare array of alias strings.
  - NBI status endpoints and `GET /rest/health/probe` return
    `text/plain` strings, not JSON objects.
...
write-mode smoke coverage to every write method:
...
- Coverage badge added to `docs/index.md` so it appears on the Read the
  Docs site as well as on GitHub and PyPI.
